...
class SearchController extends AbstractController
{
    ...
    public function index(Request $request, HttpClientInterface $client): Response
    {
        ...
        // La variable $search contient le contenu de la recherche effectuée. La clé 'search' correspond à la recherche et la clé 'keyword' aux mots-clés recherchés. Il y a un sous-tableau keyword dans le tableau search.
        $search = $data['search']['keyword'];

        // $client fait une requête vers l'API. On utilise l'interpolation avec $search, ce qui rend la requête dynamique. $apiResponse stocke le contenu de la réponse. Pour garder privée la clé API, elle est interpolée, elle est renseignée dans le fichier .env.local
        ...
        $films = $apiResponseArray['results'];
    ...
    public function getSearchBar() : Response
    {
        ...
        $form = $this->createForm(SearchType::class, null, [
            "method" => "GET",
            // Désactivation du token CSRF (cross site request forgery) qui permet d'identifier l'origine du formulaire. Cela permet d'alléger la requête. 
